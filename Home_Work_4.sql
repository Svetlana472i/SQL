...
-- Используя JOIN-ы, выполните следующие операции: Вывести всех котиков по магазинам по id (условие соединения shops.id = cats.shops_id)
SELECT shops.id AS "id магазина", shops.shopname AS "Название магазина", cats.name AS "Имя котика"
FROM shops
JOIN cats
...
SELECT * FROM shops
WHERE EXISTS 
(SELECT * FROM cats WHERE shops.id = cats.shops_id AND cats.name = "Murzik");


-- Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT shops.id AS "id магазина", shops.shopname AS "Название магазина"
FROM shops
left JOIN cats
ON shops.id = cats.shops_id
WHERE shops.id NOT IN (SELECT shops_id FROM cats WHERE cats.name = "Murzik" OR cats.name = "Zuza");
...
SELECT Orders.ord_id AS "id заказа", Orders.ord_datetime AS "Время заказа", analysis.an_name AS "Название анализа", analysis.an_price as "Цена анализа"
FROM Orders
JOIN analysis
ON Orders.ord_an = analysis.an_id
where Orders.ord_datetime between '2020-02-05 00:00:01' and '2020-02-11 23:59:59'
